...
class MaoYan():
    ...
    def __init__(self, movie_id):
        ...
        self.starturl = 'http://m.maoyan.com/review/v2/comments.json?movieId=%s&userId=-1&offset=0&limit=15&ts=%s&type=3' %(movie_id,self.starttime)
    ...
    def GetCommentNum(self):
        ...
        text = response.json()
        num1 = text['data']['total']
        num2 = text['data']['t2total']
        num=num1-num2
    ...
    def FormatUrl(self,starttime):
        ...
        url = 'http://m.maoyan.com/review/v2/comments.json?movieId=%s&userId=-1&offset=0&limit=15&ts=%s&type=3' %(self.movie_id,starttime)
        return url
    ...
    def SaveComent(self,movie_id):
        ...
        tqdm用于实现进度条
        '''
        num1 = self.GetCommentNum()
        pages = int(num1/15)
        file_name =  r"%s\comments\selected_comment%s.txt" %(get_desktop(),movie_id)
